...
function isInteresting(number, awesomePhrases) {
    function isInterestingNumber(num) {
        ...
        const incSeq = str.split('').map(Number);
        if (incSeq.join('') === Array.from({ length: incSeq.length }, (_, i) => (incSeq[0] + i) % 10).join('')) return true;

        const decSeq = str.split('').map(Number);
        if (decSeq.join('') === Array.from({ length: decSeq.length }, (_, i) => (decSeq[0] - i + 10) % 10).join('')) return true;
    ...
    for (let i = 0; i <= 2; i++) {
        ...
        if (currentNum > 99 && isInterestingNumber(currentNum)) {
            return i === 0 ? 2 : 1;
        }
    }
